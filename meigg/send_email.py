...
def sendemai():
        ...
        password = 'xzpywtrjnusrdjfg'  # 发送人的密码
...
def sendemai_fujian():
    '''发送邮件函数+附件'''
    '''第一步，创建数据'''
    smtpserver = 'smtp.qq.com'  # 定义变量，邮件服务器,exmail.qq.com,smtp.163.com
    sender = '2326268720@qq.com'  # 定义变量，发送人
    password = 'xzpywtrjnusrdjfg'  # 发送人的密码
    receiver = '1084928753@qq.com'  # 定义变量， 接收人
    '''第二步，实例化MIMEMultipart对象，在通过attach方法增加邮件正文和附件
        Header类只需要理解用来转换编码的'''
    data = MIMEMultipart()
    data.attach(MIMEText('hello,测试邮件，不用紧张，详情看附件', 'plain', 'utf-8'))  # 增加邮件正文
    data['from'] = Header(sender, 'utf-8')
    data['to'] = receiver
    data['subject'] = Header('包含附件', 'utf-8')
    '''增加附件，'''
    # 通过open方法打开一个文件，并且read（），
    att = MIMEText(open(r'C:\Users\Administrator\PycharmProjects\xin_python\POM\report\report3.html','rb').read(),"base64","utf-8")
